...
export default {
  ...
  mutations: {
    ...
    SET_MOVIE_COMMENTS: (state, movieComments) => (state.movieComment = movieComments),
  ...
  actions: {
    ...
    fetchMovieComments({commit, getters }, movieId) {
      axios({
        url: drf.articles.movie_comments(movieId),
        method: 'get',
        headers: getters.authHeader,
      })
        .then(res => {
    ...
    deleteMovieComment({ commit,getters}, {movieId, movieCommentPk}){
      if(confirm('정말 삭제하시겠습니까?')) {
        axios({
          url: drf.articles.movie_comments_delete(movieId, movieCommentPk),
          method: 'delete',
          data: {},
          headers: getters.authHeader,
        })
